...
class Timesheet extends Component {
	...
	componentDidMount() {
		...
		axios
			...
			.then((response) => {
				...
				this.setState({ oldfloatingsTaken: response.data.floatings });
				this.setState({ oldvacationsTaken: response.data.vacations });
			});
	...
	handleSave = (event) => {
		...
		let floatingLeft = 3 - this.state.floatingsTaken.length;
